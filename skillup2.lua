...
user_settings = {
    user_spells = {
        ...
        Enhancing = T{'Phalanx','Crusade'},
        Ninjutsu = T{},
        Singing = T{},
        Blue = T{'Cocoon','Pollen'},
        Summoning = T{}},
...
function precast(spell)
    ...
    if spell and spell.mp_cost > player.mp then
        if gs_skillup.skipped_spells:contains(spell.name) then
            ...
            cancel_spell()
            -- send_command('input /heal on')
			-- send_command('input //jc reset')
			send_command('input /convert')
			send_command(';wait 2.0; input /item "B.E.W. Pitaru" <me>')
			send_command(';wait 2.0;input //gs c start blue magic')
            return
        end
        cancel_spell()
